...
result = shutil.copy('../document/오픈API_품질진단결과보고서(종합)_템플릿.xlsx', '../report_combine/식품의약품안전처_종합.xlsx')
...
for report in report_list:
    ...
    wb_data = openpyxl.load_workbook('../report/' +report, data_only=True)
    ...
    error_rate = estimate_data['G20'].value
    if organ_name == '공공데이터활용지원센터':
        continue
    if oper_len == estimate_data['E14'].value and oper_len == estimate_data['F15'].value and oper_len == estimate_data['E16'].value and oper_len == estimate_data['F17'].value and oper_len == estimate_data['E18'].value:
        shutil.copy('../report/')
        continue
    ...
    combine_ws_main['B7'].value = "[오픈API 종합]"


    ##서비스진단대상
    combine_ws_service = combine_wb['서비스진단대상']
    combine_ws_service['A2'].value = '모든기관'
    ...
    for i in range(0,s1_len):
        ...
        combine_s1['O' + str(8 + i + s1_cnt)].value = report.split('_')[0]
    ...
    for i in range(0,oper_len):
        ...
        combine_s2['K' + str(8 + i + s2_cnt)].value = report.split('_')[0]
    ...
    for i in range(0, s3_len):
        ...
        combine_s3['O' + str(8 + i + s3_cnt)].value = report.split('_')[0]
    ...
    for opername in new_opernamelist5:
        ...
        combine_s5["I" + str(opercnt1)].value = report.split("_")[0]
