...
for filelink in filelinklist:
    try:
        ...
        print(filelink2)
        portal_response = requests.get(filelink2)
        soup = BeautifulSoup(portal_response.text, 'html.parser')
        filename = soup.select_one(".file-meta-table-pc tr:contains('파일데이터명') td").text
        link = filelink2
        date = str(soup.select_one("a[title='파일 다운로드']"))[84:92]
        print(date)
